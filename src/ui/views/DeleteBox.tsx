...
import { Eraser, ScanBarcode, Trash, TriangleAlert } from "lucide-react";
...
export default function MissingBox() {
  ...
  return (
    <form className="flex flex-col rounded-lg border shadow-lg p-4 m-4 gap-1 w-sm">
      ...
      <span className="italic">Remove a box from the inventory</span>
      ...
      <button className="btn btn-error self-center w-xs" type="submit" disabled={!hasBoxWithId(boxQuery)}>
        <Trash/>Delete
      </button>
    </form>
  )
